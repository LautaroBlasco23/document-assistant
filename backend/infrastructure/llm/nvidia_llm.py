...
class NvidiaRateLimiter:
    """Sliding-window rate limiter for Nvidia API requests."""

    def __init__(self, limit: int = 35, threshold: int | None = None):
        self._limit = limit
    ...
    def acquire(self) -> None:
        while True:
            with self._lock:
                ...
                logger.warning(
                    "Nvidia rate limit approaching (%d/%d req/min), throttling for %.1fs",
...
class NvidiaLLM(LLM):
    """Implements the LLM port using Nvidia's OpenAI-compatible REST API."""
    ...
    def __init__(self, config: NvidiaConfig):
        ...
        self._rate_limiter = NvidiaRateLimiter(limit=config.requests_per_minute)
    ...
    def _request(
        ...
        """POST to Nvidia with retry logic for 429s."""
        ...
        for attempt in range(max_retries):
            ...
            if resp.status_code == 429:
                ...
                logger.warning(
                    "Nvidia HTTP 429 on attempt %d/%d, retrying in %.1fs",
                    attempt + 1,
                    max_retries,
                    wait,
                )
                ...
                if task is not None:
                    ...
                    task.progress = f"Rate limited by Nvidia — retrying in {int(wait)}s"
                    time.sleep(wait)
        ...
        raise RateLimitError(provider="nvidia", retry_after=last_retry_after)
